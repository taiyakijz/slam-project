...
def gicp_register(source, target, threshold=config.ICP_THRESH, init=np.eye(4)):
...
def save_summary_plot(rows, out_name):
    ...
    plt.figure()
    bars = plt.bar(labels, values, color="#F58518")
    plt.ylabel("RMSE (m)")
    plt.title("GICP RMSE on three frame pairs")
    plt.grid(True)
...
def main():
    ...
    for label, idx_a, idx_b in PAIR_SPECS:
        ...
        T_gicp, fitness, rmse = gicp_register(
            source=frame_b["pcd_down"],
            target=frame_a["pcd_down"],
            init=np.eye(4),
        )
        if label == "later":
            out_name = "part_07_gicp_example.png"
            title = f"GICP example ({frame_a['pair_idx']} -> {frame_b['pair_idx']})"
            saved_example = save_registration_overlay(frame_b["pcd_down"], frame_a["pcd_down"], T_gicp, out_name, title)
        rows.append({
            "label": label,
    ...
    summary_path = save_summary_plot(rows, "part_07_gicp_registration_summary.png")
    for old_name in [
        "part_07_gicp_registration_overlay_early.png",
        "part_07_gicp_registration_overlay_middle.png",
        "part_07_gicp_registration_overlay_later.png",
    ]:
